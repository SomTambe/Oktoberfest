...
#define prv(a, n)  \
    fo(n) forp(a); \
    cout << endl;
#define fob(i, b,a) for (ll i = (b); i > (a); i--)
...
void solve()
{
    ...
    vl a(n+2);
    a[0] = 0;  
    vector<long double> t1(n + 2), t2(n + 3);
    fo3(i,1, n + 1) cin >> a[i]; 
    a[n + 1] = l; 
    
    fo3(i,1, n + 2)
    {
        t1[i] = 1.0 * (a[i]-a[i-1]) / i  + t1[i-1]; 
    }
    fob(i,n+1, 0)
    {
        t2[i] = 1.0 * (a[i] - a[i-1]) / (n + 2 - i) + t2[i+1];  
    }
    ...
    fo3(i,1, n+2)
    {
        if(t1[i] > t2[i + 1])
        {
            long double dis = a[i] - a[i-1]; 
            ll vf = i, vs = n + 2 - i;
            long double t = 0;  
            if(t1[i-1] >= t2[i + 1]) 
            {
                dis -= vs * (t1[i-1] - t2[i + 1]); 
                t = t1[i - 1]; 
            }
            else
            {
                dis -= vf * (t2[i + 1] - t1[i-1]); 
                t = t2[i + 1]; 
            }
            
            cout << fixed << setprecision(15) << t + 1.0 * dis / (vf + vs) << endl; 
            break; 
