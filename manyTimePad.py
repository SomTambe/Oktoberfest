...
cipher_texts = [c1, c2, c3, c4, c5, c6, c7, c8, c9, c10]
...
for cur, cipher in enumerate(cipher_texts):

    counter = collections.Counter()

    for index, cipher2 in enumerate(cipher_texts):
        if (cur != index) :
            for indexOfChar, char in enumerate(strxor(cipher.decode('hex'), cipher2.decode('hex'))):
                if char in string.printable and char.isalpha: counter[indexOfChar] += 1

    knownIndexes = []

    for ind, val in counter.items():
        if val >= 7: knownIndexes.append(ind)
 
    xor_spaces = strxor(cipher.decode('hex'), ' '*150)
    for index in knownIndexes:
        final_key[index] = xor_spaces[index].encode('hex')
        known_key_positions.add(index)

final_key_hex = ''.join([x if x is not None else '00' for x in final_key])
